...
public class ModbusTCPTransport implements ModbusTransport {
	...
	private final byte[] receive_header = new byte[HEADER_LENGTH];
	private final byte[] send_header = new byte[HEADER_LENGTH];
	...
	public int receiveFrame(ModbusMessage msg) throws IOException {
		...
		short protocol_identifier = (short) (((receive_header[2] & 0xFF) << 8) + (receive_header[3] & 0xFF));
		...
		logger.debug(Modbus.modbusMarker, "Receive: {} {}", ByteUtils.toHex(receive_header, HEADER_LENGTH), ByteUtils.toHex(msg.buff, request_body_length));
