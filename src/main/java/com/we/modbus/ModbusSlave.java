...
public class ModbusSlave extends Modbus {
	 ...
	 * 
	 * @param transport
	 *            Объект ModbusDataModel, который должен использоваться этим
	...
	public int handleRequest() throws IOException {
		...
		case READ_DISCRETE_INPUTS: {
			...
			if (request.length != 6) {
				logger.warn("ModbusSlave: Некорректная длина запроса [{}]", request.length);
				return -1;
			}
			...
			System.arraycopy(values, 0, response.buff, 3, values.length);
			// Отправляем ответ
			sendFrame(response);
			break;
		}
		...
		case WRITE_MULTIPLE_REGISTERS: {
			...
			// Проверка адреса регистров и количества
			ErrorCode errorCode = checkRefAndLen(reference, length, 7);
			if (errorCode != null) {
				sendFaultAnswer(request, errorCode);
				return -1;
			}
